...
export default class Game {
  ...
  playerHit(playerId) {
    ...
    if (player && player.status === "playing") {
      ...
      // Ajouter une méthode pour gerer le cas d'un ace, le serveur doit demander au client si l'ace doit valoir 1 ou 11
  ...
  dealerPlay() {
    ...
    while (shouldDealerHit(this.dealer.hand)) {
      ...
      if (card.rank === "A") {
        card.aceValue = calculateHandValue(this.dealer.hand) + 11 > 21 ? 1 : 11;
      }
